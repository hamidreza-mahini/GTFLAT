...
#GTFLAT>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
from utils.gtflat import *
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
class Server:
    def __init__(self, args, model, seed):
        # Set up the main attributes
        #GTFLAT>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        self.gtflat = (args.gtflat==1)
        self.gtflat_num_iterations = args.gtflat_num_iterations
        self.gtflat_num_generations = args.gtflat_num_generations
        self.gtflat_compare_layer_index = args.gtflat_compare_layer_index
        self.gtflat_cli_threshold = args.gtflat_cli_threshold
        if self.gtflat:
            gtflat_say("GTFLAT has been enabled!")
        self.top_test_acc = 0    
        #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    def get_gtew(self):
        ...
        def get_params(model):
            ...
            for param in model.parameters():
                params.append(param)
            self.gtflat_compare_layer_index = max(-len(params), self.gtflat_compare_layer_index)    
            return params[self.gtflat_compare_layer_index:]


        pd = torch.nn.PairwiseDistance()

        num_clients = self.num_users
        models_eval_matrix = np.zeros((num_clients, num_clients))
        for i, user1 in enumerate(self.selected_users):   
            ...
            for j, user2 in enumerate(self.selected_users):
                ...
                if i!=j:
                    dists=[]
                    for l1, l2 in zip(u1, u2):
                        dists.append(pd(l1, l2).norm(p='fro'))
                    models_eval_matrix[i][j] = -torch.tensor(dists).norm(p='fro')#*100       
        
        #if np.linalg.norm(models_eval_matrix) <= self.gtflat_cli_threshold * num_clients:
        #    self.gtflat_compare_layer_index -= 1
            
        gtflat_say("Comparing Layer Index: {}".format(self.gtflat_compare_layer_index))
        gtflat_say("Models Eval Matrix:\n{}".format(np.around(models_eval_matrix, 2))) 
        
        (GTEW, ASS) = ensemble( num_clients=num_clients, 
                                models_eval_matrix=models_eval_matrix, 
                                r = Replicator, 
                                num_iterations=self.gtflat_num_iterations, 
                                num_gens=self.gtflat_num_generations)
        gtflat_say("GTEW= {}".format(GTEW))
        return GTEW
    #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    def aggregate_parameters(self,partial=False):
        ...
        if self.gtflat :
            gtew =  self.get_gtew()
            gtflat_say("Aggregation Weights= {}".format(gtew))
        for i, user in enumerate(self.selected_users):
            self.add_parameters(user, user.train_samples / total_train if not self.gtflat else gtew[i],partial=partial)
    ...
    def evaluate(self, save=True, selected=False):
        ...
        if self.top_test_acc < glob_acc:
            self.top_test_acc = glob_acc
        gtflat_say("Top global accuracy: {:4f}".format(self.top_test_acc))    
        if save:
