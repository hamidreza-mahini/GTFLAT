...
def get_dataset(mode='train'):
    ...
    #transform = transform_train if mode=='train' else transform_val

    dataset = CIFAR10(root='./data/Cifar10', train=True if mode=='train' else False, download=True, transform=transform)
...
def main():
    ...
    parser.add_argument("--min_sample", type=int, default=30, help="Min number of samples per user.")
    parser.add_argument("--sampling_ratio", type=float, default=1, help="Ratio for sampling training samples.")
    parser.add_argument("--unknown_test", type=int, default=1, help="Whether allow test label unseen for each user.")
    parser.add_argument("--alpha", type=float, default=1.00, help="alpha in Dirichelt distribution (smaller means larger heterogeneity)")
